...
public class PrimeIngressosNormalizer extends ShowNormalizer {
	...
	public void run(NormalizerDTO normalizerDTO, String url) throws Exception {
		...
		String title = URLDecoder.decode(data.split("&")[1].split("=")[1], "UTF-8");
		
		String dateStr = URLDecoder.decode(data.split("&")[2].split("=")[1], "UTF-8");
		...
		String endereco = URLDecoder.decode(data.split("&")[3].split("=")[1], "UTF-8");
		...
		String local = body.select("span#ctl00_ContentPlaceHolder1_labelLocal").text();
